{ This is a (renamed) copy of the unit TAPolygon series which was added to
  TAChart in Laz 2.1x. The duplication is needed to compile the corona project
  with older Lazarus versions. }
...
type
  TcPolygonSeries = class(TBasicPointSeries)
  private
    FBrush: TBrush;
    FPen: TPen;
...
destructor TcPolygonSeries.Destroy;
begin
  FPoints := nil;
  FStart := nil;
  FPen.Free;
...
procedure TcPolygonSeries.Draw(ADrawer: IChartDrawer);
const
  START_BLOCKSIZE = 100;
var
  ext: TDoubleRect;
...
function TcPolygonSeries.GetNearestPoint(const AParams: TNearestPointParams;
...
procedure TcPolygonSeries.SetBrush(AValue: TBrush);
begin
  FBrush.Assign(AValue);
  UpdateParentChart;
end;

procedure TcPolygonSeries.SetPen(AValue: TPen);
begin
  FPen.Assign(AValue);
  UpdateParentChart;
end;
